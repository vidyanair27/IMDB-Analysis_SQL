...
SELECT SUM(ISNULL(title)) AS title_nulls,
...
SELECT year, 
	   COUNT(id) AS number_of_movies_produced_India_or_USA
FROM movie
...
SELECT COUNT(r.total_votes) AS Total_votes,
       CASE
...
ORDER BY COUNT(r.total_votes) DESC;
...
SELECT dm.name_id AS director_id,
       ...
       DATEDIFF(m.date_published, LEAD(m.date_published, 1) OVER(PARTITION BY dm.name_id ORDER BY m.date_published)) AS avg_inter_movie_days,
       r.avg_rating AS avg_rating,
       SUM(r.total_votes) AS total_votes,
